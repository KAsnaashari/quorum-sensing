	...
	for site in range(4):
		for i in range(5):
			with open(f'{ligand_name}/vina_results/3szt_{ligand_name}_site{site}_1024_{i+1}.pdbqt', 'r') as results_file:
				print(f'{ligand_name}/vina_results/3szt_{ligand_name}_site{site}_1024_{i+1}.pdbqt...')
		...
		df.to_csv(f'{ligand_name}/vina_results_sites.csv', index=False)
		print(f'Results gathered in {ligand_name}/vina_results_sites.csv')
		ligand_data.append(pd.read_csv(f'{ligand_name}/vina_results_sites.csv'))

dock_data = pd.concat(ligand_data)
dock_data.reset_index(inplace=True)
dock_data.drop('index', axis=1, inplace=True)
dock_data.to_csv('vina_results_sites.csv')
print(f'All results gathered in vina_results_sites.csv')
